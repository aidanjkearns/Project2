...
def grab_headlines():
    ...
    #html = urlopen('file:///Users/Aidan/Project2/Project2/opinion.html', context=ctx).read()      #for testing 
...
# Retrieve all of the anchor tags
    ...
    ls = list()
    for tag in tags:
        ls.append(tag.text)     #retrieve text from tags
    ls = ls[0].split('\n')      #split text to eliminate \n
    ls = ls[:-1]                #split returns blank last entry 
    return(ls)
...
def get_umsi_data():
    ...
    for page in pages:
        ...
        tags = soup('div')                 #find tags for umsi titles 
        l = list()
        for tag in tags:
            if(tag.attrs == {'class': ['field-item', 'even']}): #titles are always of class 'field-item even'
                l.append(tag.text)
        l = [ x for x in l if "\n" not in x ]    #the titles are the only text that dont have \n in them
        tags2 = soup.find_all('h2')             #find tags for umsi directory names 
        d = dict()
        t = list()
        for tag in tags2:
            if(tag.attrs == {}):            #names have empty attributes 
                t.append(tag.text)
        for index, i in enumerate(t):
            d[i] = l[index]             #create dictionary with index of the name and value from the corresponding list of titles 
        people.update(d)
    return(people)
